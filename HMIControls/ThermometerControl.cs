...
namespace HMIControls
{
    public partial class ThermometerControl : UserControl
    {
        ...
        public ThermometerControl()
        {
            ...
            //监听属性值变化
            OnTemperatureChanged += new TemperatureChanged(DoEvent_TemperatureChanged);
            InitializeComponent();
        }

        //定义一个委托
        private delegate void TemperatureChanged(object sender, EventArgs e);
        //定义一个委托关联的事件
        private event TemperatureChanged OnTemperatureChanged;

        //事件处理函数，属性值修改后的操作方法
        private void DoEvent_TemperatureChanged(object sender, EventArgs e)
        {
            //触发窗体的paint事件
            this.Invalidate();
        }
        //事件触发函数
        private void WhenTemperatureChanged()
        {
            if (OnTemperatureChanged != null)
            {
                OnTemperatureChanged(this, null);
            }
        }
